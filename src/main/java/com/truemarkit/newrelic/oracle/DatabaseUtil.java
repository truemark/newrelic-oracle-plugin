...
public class DatabaseUtil {
  ...
  @Nonnull
  public static String getJdbcUrl(@Nonnull String host,
                                  @Nonnull String port,
                                  @Nullable String sid,
  ...
  public static HikariDataSource getHikariDataSource(String name,
                                                     String host,
                                                     String port,
                                                     String sid,
                                                     String serviceName,
  ...
  public static List<ResultMetricData> getQueryResult(@Nonnull Connection conn,
    ...
    try (PreparedStatement statement = conn.prepareStatement(query);
      ...
      if (ex.getErrorCode() == 942) {
        ...
        log.error("Error executing query for " + category
            + ", table: " + tablename
  ...
  public static boolean getDatabaseStatus(@Nonnull Connection conn) {
    ...
    try (PreparedStatement statement = conn.prepareStatement(query);
      ...
      log.error("Error executing query for database status: " + e.getMessage(), e);
      return false;
    }
